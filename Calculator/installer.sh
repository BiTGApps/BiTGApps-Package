...
BITGAPPS="zip/sys/Calculator.tar.xz"
...
is_uninstaller() {
  if [ "$ZIPNAME" = "uninstall" ]; then
    ...
    rm -rf $SYSTEM_ADDOND/70-calculator.sh
    rm -rf $SYSTEM_APP/Calculator
    # End installation
    on_installed
  fi
}
...
df_reclaimed() {
  PKG="$(find /system -type d -iname '*Calculator*')"
