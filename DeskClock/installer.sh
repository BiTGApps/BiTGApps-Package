...
BITGAPPS="zip/sys/DeskClock.tar.xz"
...
is_uninstaller() {
  if [ "$ZIPNAME" = "uninstall" ]; then
    ...
    rm -rf $SYSTEM_ADDOND/70-deskclock.sh
...
sdk_v25_install() {
  ...
  tar -xf $ZIP_FILE/sys/DeskClock.tar.xz -C $TMP_SYS
  pkg_TMPSys
...
df_reclaimed() {
  PKG="$(find /system -type d -iname '*Clock*')"
