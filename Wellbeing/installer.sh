...
BITGAPPS="zip/core/Wellbeing.tar.xz zip/Permissions.tar.xz"
...
is_uninstaller() {
  if [ "$ZIPNAME" = "uninstall" ]; then
    ...
    rm -rf $SYSTEM_ADDOND/70-wellbeing.sh
...
df_reclaimed() {
  PKG="$(find /system -type d -iname '*Wellbeing*')"
